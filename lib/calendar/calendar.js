...
(function(wid, dcm) {
    ...
    function generateTitleMonth(idx, year, month, cal) {
        ...
        return new Date(year, month).getFullYear() + '年' + monthArr[cal.monthType][new Date(year, month).getMonth()];
    }

    function generateItemTitle(cal) {
        var chinese = '<span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span>';
        var english = '<span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sta</span>';
        var chineseString = cal.isSundayFirst ? ('<span>日</span>' + chinese) : (chinese + '<span>日</span>');
        var englishString = cal.isSundayFirst ? ('<span>Sun</span>' + english) : (english + '<span>Sun</span>');

        return '<div class="calendar-item-title">' + englishString + '</div>';
    ...
    Calendar.prototype = {
        ...
        initDomFuc: function() {
            ...
            html += _this.isToggleBtn ? '<span id="' + _this.container + 'CalendarTitleLeft" class="calendar-title-left">上月</span>' +
                '<span id="' + _this.container + 'CalendarTitleRight" class="calendar-title-right">下月</span>' : '';
